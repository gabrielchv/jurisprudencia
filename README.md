...
A autenticação usa Firebase e espera as seguintes variáveis de ambiente em `.env.local`:
...
- O endpoint `POST /api/search` retorna dados de exemplo no momento.
- Os tribunais, órgãos julgadores e períodos dos filtros ficam em `src/lib/constants.ts`.
